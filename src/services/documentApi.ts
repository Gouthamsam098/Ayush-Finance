...
export const documentApi = {
  async list(customerId: string): Promise<CustomerDocument[]> {
    return (await api.get<CustomerDocument[]>(`/customers/${customerId}/documents`)) ?? [];
  },

  async upload(customerId: string, type: DocumentType, file: File): Promise<CustomerDocument> {
  ...
  async remove(customerId: string, docId: string): Promise<void> {
  ...
  downloadUrl(customerId: string, docId: string): string {
