  ...
  return (
    <ResponsiveContainer width="100%" height={300}>
      <BarChart data={data} margin={{ top: 8, right: 8, left: -6, bottom: 0 }} barGap={4}>
        ...
        <Bar dataKey="disbursed" name="Disbursed" fill={C.primary} radius={[5, 5, 0, 0]} maxBarSize={26} />
        <Bar dataKey="collected" name="Collected" fill={C.success} radius={[5, 5, 0, 0]} maxBarSize={26} />
      </BarChart>
    </ResponsiveContainer>
  );
}

// ── Cash Flow Trend — Collections (green) / Expenses (red) / Net Profit (blue) ──
// `net` carries daily cumulative net profit (interest income − expenses).
...
export function CashFlowChart({ data }: { data: CashFlowPoint[] }) {
  ...
  if (!hasData) return <EmptyChart label="No cash flow in this range" height={240} />;
  return (
    <ResponsiveContainer width="100%" height={240}>
      <ComposedChart data={data} margin={{ top: 8, right: 8, left: -6, bottom: 0 }}>
        ...
        <Area type="monotone" dataKey="collections" name="Collections" stroke={C.success} strokeWidth={2.5} fill="url(#cfColl)" dot={false} activeDot={{ r: 4 }} />
        <Area type="monotone" dataKey="expenses" name="Expenses" stroke={C.danger} strokeWidth={2} fill="url(#cfExp)" dot={false} activeDot={{ r: 4 }} />
        <Area type="monotone" dataKey="net" name="Profit" stroke={C.primary} strokeWidth={2.5} fill="url(#cfNet)" dot={false} activeDot={{ r: 4 }} />
      </ComposedChart>
    </ResponsiveContainer>
  );
}
