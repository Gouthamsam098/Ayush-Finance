...
const loanColumns = `
	id, loan_number, customer_id, type, principal, rate, interest, deduction,
...
func writeArgs(in domain.LoanInput, d domain.DerivedLoan) []any {
	return []any{
		in.CustomerID, string(in.Type), in.Principal.DBRupees(), in.Rate, d.Interest.DBRupees(),
...
func (r *Repository) Create(ctx context.Context, in domain.LoanInput, d domain.DerivedLoan) (*domain.Loan, error) {
	const query = `
		INSERT INTO loans (
			loan_number, customer_id, type, principal, rate, interest, deduction,
...
func (r *Repository) Update(ctx context.Context, id int64, in domain.LoanInput, d domain.DerivedLoan) (*domain.Loan, error) {
	const query = `
		UPDATE loans SET
			customer_id=$2, type=$3, principal=$4, rate=$5, interest=$6, deduction=$7,
			disbursed_amount=$8, instalment_amount=$9, num_days=$10, loan_date=$11, next_due_date=$12, contact=$13,
			remarks=$14, vehicle_number=$15, vehicle_brand=$16, vehicle_name=$17,
			updated_at=now()
...
func scanLoan(row rowScanner) (*domain.Loan, error) {
	...
	err := row.Scan(
		&l.ID, &l.LoanNumber, &l.CustomerID, &typ, &principal, &l.Rate, &interest,
